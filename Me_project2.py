# -*- coding: utf-8 -*-
"""
Created on Mon Nov 23 16:12:14 2015
...
def right_semi_boundary (i,j):
    ...
    #print y,v
    phi [i][j] = (phi [i+1][j] + phi [i][j-1] + phi [i-1][j] - right_function(i)*h)/3.0
            ...
            for j in range (1,M-1):
                ...
                if ((i == 1) or (i == M-2) or (j == N-2) or (j == 1)):
                    if j == 1 and (i != 1 or i !=M-2):
                        ...
                        phi [i][j] = (phi [i-1][j] + phi [i][j-1] - right_function(i)*h)/3.0
                    elif i == 1 and j == N-2:
                        phi [i][j] = (phi [i+1][j] + phi [i][j-1] - right_function(i)*h)/2.0
                else:
                    interior_point (i,j)
